...
// صنف واحد داخل سلة الطلب (المطاعم تطلب عدة أصناف بكميات مختلفة).
...
export interface Order {
  ...
  recipientName?: string; // موروث من نظام الهدايا — غير مستخدم في تدفّق المطعم
  recipientPhone?: string;
  product: string;        // ملخّص نصّي للسلة
  cart?: CartItem[];      // أصناف السلة (مصدر الحقيقة أثناء المحادثة)
  price: number;          // إجمالي السلة
...
export type ConversationState =
  ...
  | 'BROWSING'
  | 'SELECTING_PRODUCT'
  | 'ADDING_ITEM_QTY'   // بعد اختيار صنف: ننتظر الكمية
  | 'CART_REVIEW'       // بعد إضافة صنف: أضف صنفاً آخر أو أنهِ الطلب
  | 'COLLECTING_NAME'
  | 'COLLECTING_PHONE'
