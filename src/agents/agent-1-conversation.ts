import { WhatsAppMessage, Product, Session, CartItem } from '../types';
...
export async function handleMessage(msg: WhatsAppMessage, shopId: string): Promise<void> {
  ...
  if (!userText) {
    if (msg.type === 'audio' || msg.type === 'voice' || msg.type === 'ptt') {
      await sendTextMessage(whatsappConfig, phone, 'عذراً، لا أستطيع الاستماع للمقاطع الصوتية حالياً. يرجى كتابة طلبك نصياً 🍽️');
    } else if (msg.type !== 'location') {
      await sendTextMessage(whatsappConfig, phone, 'عذراً، لا يمكنني معالجة هذا النوع من الملفات. يرجى مراسلتي نصياً 🍽️');
    }
    return;
  }
  ...
  if (wantsHuman) {
    const reply = 'تم إيقاف الرد الآلي وتحويل محادثتك للموظف، سيتم الرد عليك في أقرب وقت. 🍽️';
    await sendTextMessage(whatsappConfig, phone, reply);
    session.messages.push({ role: 'assistant', content: reply });
  ...
  if (asksDeliveryInfo && session.state !== 'COLLECTING_TIME') {
    ...
    const msg =
      ...
      `عند إتمام طلبك ستحدّد الوقت المناسب لك ضمن هذه الفترة، وتختار التوصيل أو الاستلام من المطعم. 🍽️`;
  ...
  const aiAnswerStates = ['BROWSING', 'SELECTING_PRODUCT', 'ADDING_ITEM_QTY', 'CART_REVIEW', 'COLLECTING_NAME', 'COLLECTING_PHONE', 'COLLECTING_FULFILLMENT', 'COLLECTING_TIME', 'CONFIRMING_ORDER'];
  ...
  switch (session.state) {
    ...
    case 'SELECTING_PRODUCT':
      ...
      break;
    case 'ADDING_ITEM_QTY':
      await handleQuantity(phone, shopId, whatsappConfig, userText, intent, session);
      break;
    case 'CART_REVIEW':
      await handleCartReview(phone, shopId, whatsappConfig, userText, intent, session);
      break;
...
async function handleGreeting(
  ...
  const greetingText = `أهلاً وسهلاً بك في ${shopName}! 🍽️\n\nيسعدنا خدمتك. تفضّل بالاطّلاع على قائمة طعامنا 👇`;
...
async function handleProductSelection(
  ...
  // Extract a product number even from phrasing like "رقم 1" / "الصنف ٢" / "ابغى 3".
  ...
  if (!selected.available || (selected.stock !== undefined && selected.stock <= 0)) {
    const outOfStockMsg = `عذراً، الصنف (${selected.name}) غير متوفّر حالياً! ❌\n\nيرجى اختيار صنف آخر من القائمة.`;
    await sendTextMessage(whatsappConfig, phone, outOfStockMsg);
    session.messages.push({ role: 'assistant', content: outOfStockMsg });
    return;
  }

  await askItemQuantity(phone, whatsappConfig, session, selected);
}

// يعرض بطاقة الصنف المختار ويطلب الكمية ثم ينتقل إلى ADDING_ITEM_QTY.
async function askItemQuantity(
  ...
  const confirmation = `✨ *اختيار موفّق!* ✨\n` +
    `━━━━━━━━━━━━━━\n` +
    `🍽️ *${selected.name}*\n` +
    (selected.description ? `📝 ${selected.description}\n` : '') +
    `💰 *السعر: ${formatPrice(selected.price)}*\n` +
    `━━━━━━━━━━━━━━\n\n` +
    `كم الكمية المطلوبة من هذا الصنف؟ (أرسل رقماً، أو اكتب «واحد») 🔢`;
  ...
  session.state = 'ADDING_ITEM_QTY';
}

// يلتقط الكمية للصنف المختار ويضيفه للسلة ثم يعرض مراجعة السلة.
async function handleQuantity(
  ...
  if (!selected) {
    // فقدنا الصنف المختار — أعد العميل للقائمة.
    session.state = 'SELECTING_PRODUCT';
    const catalogResult = await sendProductCatalog(shopId, whatsappConfig, phone, sendTextMessage, sendImageMessage);
    session.messages.push({ role: 'assistant', content: catalogResult });
    return;
  ...
  // حوّل الأرقام العربية ثم استخرج الكمية. كلمات مثل «واحد/حبة/وحده» = 1.
  const normalized = text.replace(/[٠-٩]/g, (d) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)));
  const numMatch = normalized.match(/\d{1,3}/);
  let qty = numMatch ? parseInt(numMatch[0], 10) : NaN;
  if (isNaN(qty) && /واحد|وحده|وحدة|حبة|حبه|وجبة|وجبه/.test(text)) qty = 1;

  if (isNaN(qty) || qty < 1) {
    await sendTextMessage(whatsappConfig, phone, 'يرجى إرسال الكمية كرقم (مثال: 1 أو 2). 🔢');
    return;
  }
  if (qty > 50) qty = 50; // حد أعلى منطقي للكمية الواحدة

  const cart: CartItem[] = session.orderData.cart || [];
  addToCart(cart, selected, qty);
  session.orderData.cart = cart;
  session.selectedProduct = undefined;

  await sendCartReview(phone, whatsappConfig, session, `تمت إضافة *${selected.name}* ×${qty} إلى سلتك ✅`);
}

// يعرض محتوى السلة الحالي ويطلب: إضافة صنف آخر أو إنهاء الطلب.
async function sendCartReview(
  phone: string,
  whatsappConfig: WhatsAppConfig,
  session: Session,
  prefix: string
  ...
  const msg =
    ...
    `• لإضافة صنف آخر: أرسل *رقم الصنف* من القائمة\n` +
    `• لعرض القائمة من جديد: اكتب *القائمة*\n` +
    `• لإتمام الطلب: اكتب *إنهاء* ✅`;
  await sendTextMessage(whatsappConfig, phone, msg);
  session.messages.push({ role: 'assistant', content: msg });
  session.state = 'CART_REVIEW';
}

// مراجعة السلة: إنهاء الطلب، عرض القائمة، أو إضافة صنف آخر.
async function handleCartReview(
  ...
  // إنهاء الطلب / الدفع
  const wantsCheckout = intent.intent === 'checkout' ||
    /إنهاء|انهاء|إتمام|اتمام|خلص|خلصت|كفى|كفي|تم$|^تم |الدفع|اطلب|اطلبها|اكمل|أكمل|كملنا|بس كذا|بس كده|هذا كل شيء|هذا كل شي/.test(t);
  if (wantsCheckout) {
    if (cart.length === 0) {
      await sendTextMessage(whatsappConfig, phone, 'سلتك فارغة 🧺 — أرسل رقم صنف من القائمة لإضافته أولاً.');
      return;
    }
    await askCustomerName(phone, whatsappConfig, session);
    return;
  }

  // عرض القائمة من جديد
  if (/القائمة|القائمه|المنيو|المنيو|قائمة|menu/.test(t)) {
    const catalogResult = await sendProductCatalog(shopId, whatsappConfig, phone, sendTextMessage, sendImageMessage);
    session.messages.push({ role: 'assistant', content: catalogResult });
    session.state = 'SELECTING_PRODUCT';
    return;
  ...
  // محاولة تفسير الرسالة كاختيار صنف آخر (رقم أو اسم)
  const products = await getAllProducts(shopId);
  let selected: Product | null = null;
  const normalized = text.replace(/[٠-٩]/g, (d) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)));
  const numMatch = normalized.match(/\d{1,2}/);
  ...
  if (selected) {
    if (!selected.available || (selected.stock !== undefined && selected.stock <= 0)) {
      await sendTextMessage(whatsappConfig, phone, `عذراً، الصنف (${selected.name}) غير متوفّر حالياً. اختر صنفاً آخر أو اكتب *إنهاء*.`);
      return;
    }
    await askItemQuantity(phone, whatsappConfig, session, selected);
    return;
  ...
  // لم نفهم — أعد عرض السلة مع الخيارات.
  await sendCartReview(phone, whatsappConfig, session, 'لم أفهم طلبك 🤔');
}

// يطلب اسم العميل لبدء جمع بيانات التسليم (بعد إنهاء السلة).
async function askCustomerName(
  phone: string,
  whatsappConfig: WhatsAppConfig,
  session: Session
): Promise<void> {
  const reply = `ممتاز! لإتمام طلبك أحتاج بعض المعلومات.\nما اسمك الكريم؟ 🙏`;
  await sendTextMessage(whatsappConfig, phone, reply);
  session.messages.push({ role: 'assistant', content: reply });
...
async function askFulfillmentOptions(
  ...
  // بناء خيارات الاستلام حسب إعدادات المطعم
...
async function sendOrderSummary(
  ...
  const fulfillmentLine = isPickup ? '🏬 الاستلام: من المطعم' : '🚚 التوصيل: إلى موقعك';
...
async function handleConfirmation(
  ...
  if (intent === 'confirm' || lower === 'نعم' || lower === 'اي' || lower === 'تمام' || lower === 'أكيد') {
    if (shop.subscriptionPlan === 'SILVER') {
      ...
      if (ordersCount >= 500) {
        await sendTextMessage(whatsappConfig, phone, 'عذراً، وصل المطعم للحد الأقصى للطلبات هذا الشهر. يرجى مراجعة إدارة المطعم لاحقاً.');
        return;
      }
    }
...
async function proceedWithPaymentMethod(
  ...
  // اشتقّ ملخّص السلة والإجمالي من سلة الطلب (مصدر الحقيقة).
  const cart: CartItem[] = session.orderData.cart || [];
  ...
  session.orderData.product = summary;
  session.orderData.price = total;
  ...
  // احفظ سطور السلة في جدول OrderItem للسجل التفصيلي (اللوحة/التقارير).
  if (cart.length > 0) {
    try {
      ...
      logger.error(`[Agent1] Failed to persist order items for ${orderId}: ${e.message}`);
    }
  }
  ...
  if (methodId === 'ONLINE') {
    ...
    await sendTextMessage(whatsappConfig, phone, 'بعد التحويل، أرسل كلمة *تم التحويل* لكي نتمكن من مراجعة الطلب وتأكيده. 🍽️');
    ...
    const summaryMsg =
      ...
      `رقم الطلب: ${orderId}\n` +
      `🧺 طلبك: ${summary}\n` +
      `المبلغ: ${total} ريال (الدفع عند الاستلام)\n` +
      (isPickup ? `📦 الاستلام: من المطعم${timeLine}` : `🚚 التوصيل: إلى موقعك${timeLine}`) +
      `\n\nشكراً لطلبك من ${shop.name}! 🍽️`;
    ...
    const adminMsg =
      ...
      `🍽️ Order: ${summary || 'N/A'}\n` +
      `💰 Price: ${total} SAR (CASH)\n` +
      `${isPickup ? '🏬 الاستلام: من المطعم' : '🚚 التوصيل: إلى الموقع'}${session.orderData.preferredTime ? ' | الوقت: ' + session.orderData.preferredTime : ''}\n` +
...
async function handleWithAI(
  ...
  const stepHint: Record<string, string> = {
    SELECTING_PRODUCT: 'العميل يتصفّح القائمة؛ بعد الإجابة شجّعه على اختيار رقم الصنف لإضافته للسلة.',
    ADDING_ITEM_QTY: 'نحن بانتظار كمية الصنف المختار؛ بعد الإجابة اطلب الكمية برقم.',
    CART_REVIEW: 'العميل لديه سلة قيد البناء؛ شجّعه على إضافة أصناف أخرى أو كتابة «إنهاء» لإتمام الطلب.',
  ...
  const productContext =
    ...
    `أصناف القائمة المتاحة وأسعارها:\n${await formatProductList(shopId)}\n` +
...
async function handleOwnerMessage(
  ...
  if (msg.type === 'image' && msg.image?.buffer) {
    ...
    await sendTextMessage(whatsappConfig, phone, "تم استلام صورة الصنف بنجاح! 📸\n\nما هو اسم هذا الصنف؟");
    return;
  }
  ...
  if (text.trim() === 'إلغاء' || text.trim() === 'الغاء') {
    session.state = 'GREETING';
    session.orderData = {};
    await sendTextMessage(whatsappConfig, phone, "تم إلغاء إضافة الصنف. يمكنك إرسال صورة جديدة في أي وقت للإضافة.");
    return;
  }
  ...
  switch (session.state) {
    ...
    case 'OWNER_COLLECTING_PRODUCT_DESC':
      ...
      try {
        ...
        await sendTextMessage(whatsappConfig, phone, "🎉 تم إضافة الصنف إلى القائمة بنجاح!\nيمكنك إرسال صورة صنف آخر لإضافته.");
      } catch (e: any) {
        logger.error(`[Owner Flow] Failed to save product: ${e.message}`);
        await sendTextMessage(whatsappConfig, phone, "حدث خطأ أثناء حفظ الصنف. يرجى المحاولة مرة أخرى.");
      }
    ...
    default:
      await sendTextMessage(whatsappConfig, phone, "أهلاً بك يا مدير المطعم 👑\nلإضافة صنف جديد إلى القائمة، فقط أرسل صورته هنا وسأساعدك في إضافته.");
      break;
  }
}
