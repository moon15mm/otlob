...
export function initTelegramBot(token: string): void {
  ...
  bot.onText(/\/start/, async (msg: any) => {
    ...
    if (shop) {
      bot.sendMessage(chatId,
        `مرحباً ${firstName}! مطعمك: *${shop.name}*\n\n` +
        `📸 أرسل صورة صنف لإضافته\n` +
        `📋 /products — قائمة الأصناف\n` +
        `⚙️ /settings — إعدادات المطعم\n` +
        `❌ /cancel — إلغاء`,
        { parse_mode: 'Markdown' }
      );
      return;
  ...
  bot.onText(/\/link (.+)/, async (msg: any, match: any) => {
    ...
    try {
      ...
      bot.sendMessage(chatId,
        `🎉 *أهلاً بك في بوت الإدارة الخاص بمطعمك!*\n\n` +
        `✅ تم ربط حسابك بنجاح بمطعم: *${shop.name}*\n\n` +
        `يمكنك الآن البدء في بناء القائمة الخاص بك بسهولة. ` +
        `كل ما عليك فعله هو إرسال صورة للصنف، وسأقوم بسؤالك عن تفاصيله (الاسم، السعر، الوصف).\n\n` +
        `📌 *الأوامر المتاحة لك:*\n` +
        `📋 /products — عرض قائمة أصنافك الحالية\n` +
  ...
  bot.onText(/\/products/, async (msg: any) => {
    ...
    if (!products.length) {
      bot.sendMessage(chatId, '📭 لا توجد أصناف بعد.\n\nأرسل صورة صنف لإضافته!');
      return;
    }
    ...
    bot.sendMessage(chatId,
      `📋 *أصناف ${shop.name}* (${products.length})\n\n${list}`,
      { parse_mode: 'Markdown' }
    );
  });
  ...
  bot.onText(/\/cancel/, (msg: any) => {
    ...
    bot.sendMessage(chatId, '❌ تم الإلغاء. أرسل صورة صنف جديد في أي وقت.');
  });
  ...
  // -------------------------------------------------------------------
  // استقبال النصوص (دورة إضافة الصنف)
  // -------------------------------------------------------------------
  bot.on('text', async (msg: any) => {
    ...
    if (!session) {
      const shop = await prisma.shop.findFirst({ where: { ownerTelegramId: chatId } });
      if (shop) {
        bot.sendMessage(chatId, '📸 أرسل صورة صنف لإضافته\n⚙️ /settings — إعدادات المطعم\n📋 /products — قائمة الأصناف');
      } else {
    ...
    switch (session.state) {
      case 'WAITING_NAME':
        ...
        bot.sendMessage(chatId, `✅ الاسم: *${text}*\n\n💰 كم سعر الصنف؟ (رقم فقط مثل: 50)`, { parse_mode: 'Markdown' });
        break;
      ...
      case 'WAITING_STOCK': {
        ...
        try {
          ...
          bot.sendMessage(chatId,
            ...
            `🔢 الكمية: ${stock}\n\n` +
            `أرسل صورة صنف آخر، أو /products لعرض القائمة.`,
            { parse_mode: 'Markdown' }
          );
        } catch (e: any) {
          logger.error(`[Telegram] Product save error: ${e.message}`);
          bot.sendMessage(chatId, '❌ فشل حفظ الصنف. يرجى المحاولة مجدداً.');
          ownerSessions.delete(chatId);
        }
        break;
      }
