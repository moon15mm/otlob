...
export async function sendEmail(to: string, subject: string, html: string, text?: string): Promise<void> {
  ...
  const smtp = getSmtp();
  const from = smtp.from || smtp.user || 'no-reply@otlob.xyz';
...
export async function sendPasswordResetEmail(
  ...
  const subject = 'إعادة تعيين كلمة المرور | منصة اطلب';
  const text =
    `مرحباً ${shopName},\n\n` +
  ...
  <div dir="rtl" style="font-family: Tahoma, Arial, sans-serif; background:#0f0c20; padding:30px; color:#f5f3f7;">
    <div style="max-width:480px; margin:auto; background:#15102a; border-radius:16px; padding:30px; border:1px solid rgba(255,255,255,0.07);">
      <h2 style="background:linear-gradient(to left,#ff2d81,#8e2de2); -webkit-background-clip:text; background-clip:text; color:#ff2d81; margin:0 0 10px;">اطلب | OTLOB</h2>
