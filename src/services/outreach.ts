...
export async function generateOutreachDrafts(
  ...
  const system = `أنت مساعد مبيعات محترف لمنصة "اطلب" (${baseUrl()}) — منصة SaaS تحوّل المطاعم إلى نظام طلبات آلي على واتساب: ردّ آلي 24/7، عرض قائمة الطعام واستقبال الطلبات وجمع بيانات العميل والموقع، روابط دفع فورية، ولوحة تحكم بتحليلات.

مهمتك: صياغة رسائل تواصل (Outreach) موجّهة لأصحاب المطاعم لإقناعهم بتجربة المنصة.

القواعد:
- اللهجة عربية احترافية ودودة (يمكن لمسة خليجية خفيفة)، مختصرة جداً ومناسبة للواتساب.
- مخصّصة باسم المطعم، وبدون مبالغات أو وعود غير واقعية، وبدون أسلوب سبام.
...
اكتب نصّين جاهزين للإرسال عبر واتساب.`;
  ...
  try {
    ...
    const fallback =
      `مرحباً بكم في ${prospect.name} 🍽️\n` +
      `نوفّر لكم عبر منصة "اطلب" موظّف مبيعات آلي على واتساب يرد على عملائكم 24/7، يستقبل الطلبات ويجمع الموقع، ويرسل رابط دفع فوري.\n` +
