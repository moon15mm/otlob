...
const DISCOVERY_QUERIES = [
  'مطعم', 'مطاعم وجبات سريعة', 'مطعم برجر', 'مطعم بيتزا',
  'مطعم شاورما', 'كافيه ومطعم', 'restaurant', 'fast food restaurant',
];
...
export async function discoverLeads(city: string): Promise<Lead[]> {
  ...
  agentLog(`[بدء] بحث موسّع عن المطاعم في «${city}» عبر Places API (New)...`);
  try {
    ...
    agentLog(`[انتهاء] ${leads.length} مطعم من خرائط Google. ملاحظة: المحلات الموجودة على انستقرام فقط لا تظهر هنا — استخدم روابط البحث اليدوي.`);
    return leads;
  } catch (err: any) {
...
export async function sendViaBot(prospectId: string, senderShopId: string, message: string): Promise<void> {
  ...
  if (!prospect.phone) throw new Error('لا يوجد رقم واتساب لهذا المطعم');
  ...
  agentLog(`[إرسال] رسالة إلى «${prospect.name}» (${to}) عبر مطعم «${shop.name}».`);
}
...
export async function runCycle(): Promise<void> {
  ...
  try {
    ...
    agentLog(`[معلومة] ${due.length} مطعم بحاجة متابعة (مرّ ${s.followupDays} أيام أو أكثر).`);
