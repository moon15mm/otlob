...
export async function formatProductList(shopId: string): Promise<string> {
  ...
  if (available.length === 0) {
    return 'لا توجد أصناف متاحة حالياً في هذا المطعم.';
  }
  let list = '🍽️ *قائمة الطعام (المنيو)* 🍽️\n';
  list += '━━━━━━━━━━━━━━━━━━━\n\n';
  ...
  list += '━━━━━━━━━━━━━━━━━━━\n';
  list += '✅ *اختر رقم الصنف* أو اكتب اسمه لإضافته لسلتك';
  return list;
}
...
export async function sendProductCatalog(
  ...
  if (available.length === 0) {
    const msg = 'لا توجد أصناف متاحة حالياً في هذا المطعم.';
  ...
  available.forEach((p, i) => {
    const stockText = (p.stock !== undefined && p.stock <= 3 && p.stock > 0)
      ...
      : (p.stock !== undefined && p.stock <= 0 ? ' ❌ _غير متوفّر حالياً_' : '');
    list += `*${i + 1}.* 🍽️ *${p.name}* — *${p.price} ريال*${stockText}\n`;
    if (p.description) list += `     📝 ${p.description}\n`;
    list += `\n`;
  });
  list += '━━━━━━━━━━━━━━━━━━━\n✅ *أرسل رقم الصنف* لإضافته لسلتك';

  // Send: one image (collage) then one text list. Falls back to text-only.
  if (collageUrl) {
    try {
      await sendImageFn(whatsappConfig, phone, collageUrl, '🍽️ *إليك قائمة طعامنا* 👇');
    } catch {
  ...
  return `عرض قائمة الطعام (${available.length} صنف)`;
}
