...
router.post('/auth/forgot-password', async (req, res) => {
  ...
  try {
    ...
    if (shop) {
      ...
      // use the server-configured base URL (defaults to https://otlob.xyz).
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (!entry) {
        entry = {
          shopId: r.shopId,
          shopName: shopName.get(r.shopId) || '(مطعم محذوف)',
          totalTokens: 0,
          totalRequests: 0,
          totalCostUsd: 0,
...
router.post('/admin/shops', authenticateSuperAdmin, async (req, res) => {
  ...
  if (!name || !subdomain || !username || !password) {
    return res.status(400).json({ error: 'يرجى تقديم اسم المطعم، الدومين الفرعي، اسم المستخدم وكلمة المرور' });
  }

  // Email is required so the shop owner can recover a forgotten password.
  if (!isValidEmail(email)) {
    return res.status(400).json({ error: 'يرجى تقديم بريد إلكتروني صالح للمطعم (مطلوب لاستعادة كلمة المرور)' });
  }
  ...
  try {
    ...
    if (existingShop) {
      return res.status(400).json({
        error: 'اسم المستخدم أو الدومين الفرعي أو البريد الإلكتروني مسجل مسبقاً لمطعم آخر',
      });
    }
    ...
    if (whatsappPhoneId) {
      ...
      if (existingPhone) {
        return res.status(400).json({
          error: 'رقم الواتساب مسجل مسبقاً لمطعم آخر',
        });
      }
    }
...
router.delete('/admin/shops/:id', authenticateSuperAdmin, async (req, res) => {
  try {
    ...
    res.json({ message: 'تم حذف المطعم وبياناته بنجاح' });
  } catch (err: any) {
    logger.error(`[API] ${req.method} ${req.originalUrl}: ${err.message}`);
    res.status(500).json({ error: 'حدث خطأ في الخادم. يرجى المحاولة لاحقاً.' });
  }
...
router.put('/admin/shops/:id', authenticateSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'تم تحديث خطة اشتراك المطعم بنجاح', shop: safeUpdated });
  } catch (err: any) {
    logger.error(`[API] ${req.method} ${req.originalUrl}: ${err.message}`);
    res.status(500).json({ error: 'حدث خطأ في الخادم. يرجى المحاولة لاحقاً.' });
  }
...
router.post('/admin/prospects', authenticateSuperAdmin, async (req, res) => {
  ...
  if (!name || !String(name).trim()) {
    return res.status(400).json({ error: 'يرجى إدخال اسم المطعم المستهدف' });
  }
  try {
...
router.post('/admin/agent/discover-leads', authenticateSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'حدث خطأ أثناء اكتشاف المطاعم' });
  }
});
...
router.post('/admin/agent/send-message', authenticateSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!shop) {
      return res.status(404).json({ error: 'مطعم الإرسال غير موجود' });
    }
    ...
    if (!active) {
      return res.status(400).json({ error: 'جلسة واتساب لمطعم الإرسال غير متصلة حالياً' });
    }
...
router.put('/shop/details', authenticateShop, async (req, res) => {
  ...
  try {
    ...
    if (!shop) return res.status(404).json({ error: 'المطعم غير موجود' });
    ...
    if (typeof email === 'string' && email.trim() !== '') {
      ...
      if (clash) {
        return res.status(400).json({ error: 'هذا البريد الإلكتروني مستخدم من قبل مطعم آخر.' });
      }
    ...
    res.json({ message: 'تم تحديث إعدادات المطعم بنجاح' });
  } catch (err: any) {
    logger.error(`[API] ${req.method} ${req.originalUrl}: ${err.message}`);
    res.status(500).json({ error: 'حدث خطأ في الخادم. يرجى المحاولة لاحقاً.' });
  }
      ...
      line_items: [
        {
          price_data: {
            currency: 'sar',
            product_data: {
              name: `تجديد اشتراك المطعم - باقة ${plan === 'SILVER' ? 'الفضية' : plan === 'GOLD' ? 'الذهبية' : 'البلاتينية'}`,
...
router.post('/shop/products', authenticateShop, async (req, res) => {
  ...
  if (!name || price === undefined) {
    return res.status(400).json({ error: 'يرجى تقديم اسم الصنف وسعره' });
  }
...
router.put('/shop/products/:id', authenticateShop, async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res.status(404).json({ error: 'الصنف غير موجود أو لا تملك صلاحية تعديله' });
    }
...
router.delete('/shop/products/:id', authenticateShop, async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res.status(404).json({ error: 'الصنف غير موجود أو لا تملك صلاحية حذفه' });
    }
...
router.post('/shop/products/:id/status', authenticateShop, async (req, res) => {
  ...
  try {
    ...
    res.json({ message: `تم نشر الصنف كحالة واتساب بنجاح! تم إرسالها إلى ${count} عميل.` });
  } catch (err: any) {
...
router.get('/shop/analytics', authenticateShop, async (req, res) => {
  ...
  try {
    ...
    const productProjections = products.map((p) => {
      ...
      if (p.stock <= 3) {
        restockAlerts.push(`⚠️ الصنف (${p.name}) مخزونه منخفض جداً: متبقي فقط ${p.stock} حبة في المخزن.`);
